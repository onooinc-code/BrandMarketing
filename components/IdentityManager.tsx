...
const LogoGeneratorSection: React.FC<LogoGeneratorSectionProps> = ({ logoName, savedLogo, onSave }) => {
    ...
    const generateLogoImage = useCallback(async () => {
        ...
        try {
             const ai = new GoogleGenAI({ apiKey: process.env.API_KEY as string });
             const prompt = `Generate a professional logo based on the following detailed description. The logo should be on a transparent background, focusing on the symbol or wordmark itself. Do not add any extra text like the company name unless it's part of the logo wordmark itself. Description: "${detailedPrompt}"`;
    ...
    const handleDownload = () => {
        if(savedLogo) {
            downloadImage(savedLogo, `${logoName}-logo.png`);
        }
    }

    const currentLogo = generatedLogo || savedLogo;
    ...
    return (
        <div className="w-full p-6 bg-gray-700/50 rounded-lg shadow-inner space-y-4">
            ...
            <div>
                ...
                <textarea value={initialPrompt} onChange={e => setInitialPrompt(e.target.value)} placeholder={`مثال: شعار يرمز للنمو والبيانات لشركة برمجيات`} className="w-full p-2 bg-gray-800 border border-gray-600 rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none" rows={2} />
            </div>
            <button onClick={generateDetailedPrompt} disabled={loadingState === 'prompt' || !initialPrompt.trim()} className="w-full py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-gray-500">
            ...
            <div className="mt-4 flex justify-center items-center h-40 bg-gray-900/50 rounded-md">
                ...
                {currentLogo && <img src={currentLogo} alt={`${logoName} logo`} className="max-h-36 object-contain" />}
            </div>
            ...
            {isSaved && 
                <div className="text-center">
                    <p className="text-green-400 font-bold">تم حفظ الشعار بنجاح!</p>
                    <button onClick={handleDownload} className="mt-2 px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-500">تحميل الشعار</button>
                </div>
            }
...
const IdentityManager: React.FC<{ 
    onLogoSave: (logoType: 'brandErp' | 'onoo', imageUrl: string) => void,
    savedLogos: { brandErp: string | null; onoo: string | null; }
}> = ({ onLogoSave, savedLogos }) => {
    return (
        <div className="flex flex-col items-center">
            ...
            <div className="w-full grid md:grid-cols-2 gap-8">
                <LogoGeneratorSection logoName="BrandERP" savedLogo={savedLogos.brandErp} onSave={(url) => onLogoSave('brandErp', url)} />
                <LogoGeneratorSection logoName="Onoo" savedLogo={savedLogos.onoo} onSave={(url) => onLogoSave('onoo', url)} />
            </div>
        </div>
    );
};
